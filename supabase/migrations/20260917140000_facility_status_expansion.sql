-- PostgreSQL requires enum values added in a transaction to be committed before
-- later statements may reference them. Keep this enum phase separate from the
-- constraint and function replacements below.
begin;

alter type public.facility_status_type add value if not exists 'courts_full';
alter type public.facility_status_type add value if not exists 'courts_wet_unsafe';
alter type public.facility_status_type add value if not exists 'maintenance';
...
alter table public.facility_statuses
  drop constraint facility_statuses_fixed_expiry;

alter table public.facility_statuses
  add constraint facility_statuses_fixed_expiry
  check (
